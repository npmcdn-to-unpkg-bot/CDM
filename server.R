  ...
  observe({ 
    ...
    if(!is.null(input$check_display_wb)){
      ...
      output$cargois_gy_distance <- renderPlot({
        # if(length(unique(versus_data_cargois()$dist)) == 1){
        #   p_gy_distance <- ggplot(data = versus_data_cargois(), 
        #                           aes(x = dist, 
...
######### Seabury evolution plot #########
  ...
  # 0. updateselectizeinupe
  updateSelectizeInput(session = session, inputId = 'fr24_select_ac_type',
                       choices = choice.ACtype.fr24, server = TRUE)
  updateSelectizeInput(session = session, inputId = 'fr24_airline.code',
                       choices = choice.bio.airport, server = TRUE)
...
WHERE COMPLETION_STATUS=0\n"
    ...
    if(!is.null(input$fr24_airline.code)){
      airlines <- gsub(",.*$", "", input$fr24_airline.code)
      
      # select_airline <- paste("AND AIRLINE = '", airlines,"'\n", sep = '')
      airlines_ <- paste(airlines, collapse = "' OR AIRLINE = '")
      select_airline <- paste("AND (AIRLINE = '", airlines_, "')\n", sep = '') 
    ...
    if(!is.null(input$fr24_select_ac_type)){
      ...
      ac_ <- paste(input$fr24_select_ac_type, collapse = "' OR AC_TYPE = '")
      select_ac_type <- paste("AND (AC_TYPE = '", ac_, "')\n", sep = '') 
      # Finish the query string
    } else {
      select_ac_type <- ''
  ...
  output$fr24_airline_network_map <- renderLeaflet({
    ...
    #                  "Seattle, WA 98138"
    # )
    AP <- data.table(AIRPORT_LOCATION())[, 1:4, with = FALSE]
